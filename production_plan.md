...
> **Status: ✅ Complete.** Steps 3.1–3.7 are all done. See `HANDOFF.md` for the as-built summary.
...
| 3.6 | **Add Spring Cloud Gateway** as the single entry point for all REST traffic. JWT validation, rate limiting, request logging. *(As built: new `api-gateway` module on port 8080 using `spring-cloud-starter-gateway-server-webflux` (reactive/Netty). Routes to all 7 backend services. Reactive `SecurityWebFilterChain` validates IAM-issued RS256 JWTs via JWKS. Redis-backed `RequestRateLimiter` at 20 req/s per principal with burst 40. `RequestLoggingFilter` global filter. Actuator/Prometheus/Zipkin/structured JSON logging.)* ✅ done |
| 3.7 | **Implement mTLS** for internal service-to-service REST calls. *(As built: per-service PKCS12 keystores generated by `certs/generate-certs.sh`, signed by SmartHome Dev CA. All 8 services enable HTTPS via `server.ssl.*` with `client-auth: need`. RestClient in gateway/simulator uses `RestClientSsl` with SSL bundle `internal`. API Gateway routes to HTTPS backends via `SPRING_CLOUD_GATEWAY_HTTPCLIENT_SSL_SSLBUNDLE`. JVM-level `javax.net.ssl.trustStore` set via `JAVA_TOOL_OPTIONS`. Prometheus scrapes via `scheme: https` with `insecure_skip_verify`. Healthchecks use `curl -fk https://...`.)* ✅ done |
