...
**Status:** Phases 1, 2, 3A, 3B, 3C, 3.4/3.5, 3.6, and 3.7 complete — Phase 3 is done. mTLS secures all internal service-to-service REST communication. Full Maven reactor (9 modules) builds green; all unit/security tests pass (26 tests across 6 security test classes) and Docker-dependent integration tests skip cleanly when no daemon is present.
...
- ✅ **Phase 3.7 — mTLS:** Per-service PKCS12 keystores generated by `certs/generate-certs.sh` (signed by SmartHome Dev CA with `extendedKeyUsage=serverAuth,clientAuth`). All 8 services enable HTTPS via `server.ssl.*` with `client-auth: need`. RestClient in gateway/simulator uses `RestClientSsl` with SSL bundle `internal` (`spring-boot-restclient` dependency). API Gateway routes to HTTPS backends via `SPRING_CLOUD_GATEWAY_HTTPCLIENT_SSL_SSLBUNDLE`. JVM-level `javax.net.ssl.trustStore` set via `JAVA_TOOL_OPTIONS` for JWKS and outbound SSL trust. Prometheus scrapes via `scheme: https` with `insecure_skip_verify`. Healthchecks use `curl -fk https://...`. Defaults to disabled for local dev.
- ✅ **Phase 4 — Kubernetes & CI/CD:** Health probes (liveness/readiness groups) configured per-service with dependency-appropriate indicators. Dockerfiles optimized with Spring Boot layered JARs (3-stage build: compile → extract layers → runtime). Helm umbrella chart (`helm/smarthome/`) with reusable `spring-service` sub-chart for all 8 services; per-environment values (dev/staging/prod); RabbitMQ Cluster Operator CR; HPA for gateway/processing/api-gateway. GitHub Actions CI/CD pipeline (`.github/workflows/ci-cd.yml`): lint (checkstyle) → unit test → integration test (Testcontainers) → build & push to GHCR (matrix 8 services) → Trivy vulnerability scan → deploy staging (manual environment gate) → deploy production (manual approval).
...
**Common env wiring:** `RABBITMQ_HOST/PORT/USERNAME/PASSWORD`, `RABBITMQ_SSL_ENABLED/TRUSTSTORE/TRUSTSTORE_PASSWORD` (Phase 3C), `SSL_ENABLED/SSL_KEYSTORE/SSL_KEYSTORE_PASSWORD/SSL_CLIENT_AUTH/SSL_TRUSTSTORE/SSL_TRUSTSTORE_PASSWORD` (Phase 3.7 mTLS), `JAVA_TOOL_OPTIONS` (JVM truststore for CA), `REDIS_HOST/PORT`, `DB_HOST/PORT/NAME/USERNAME/PASSWORD`, `IAM_ISSUER` (HTTPS in Docker), `ZIPKIN_ENDPOINT=http://zipkin:9411/api/v2/spans`. Each RabbitMQ-using service has its own per-service credentials and mounts `./certs:/certs:ro` for TLS. All app services expose `/actuator/health` healthchecks via HTTPS (`curl -fk`) with a 60s start period. All resource servers depend on `iam-service: condition: service_healthy`. Named volumes persist RabbitMQ, Redis, TimescaleDB, Prometheus, and Grafana data.
...
1. **Remaining Phase 4 items** — TimescaleDB Helm chart or managed PostgreSQL (4.3), observability stack via kube-prometheus-stack (4.4), HashiCorp Vault or Sealed Secrets (4.5), Istio service mesh (4.7), load testing with k6/Gatling (4.8).
2. **Frontend dashboard** — React/Next.js dashboard (see `frontend_plan.md`).
